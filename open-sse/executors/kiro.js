...
export class KiroExecutor extends BaseExecutor {
  ...
  getOrderedBaseUrls(credentials) {
    ...
    // Region-aware: derive the runtime region from the profileArn (or the stored
    // region) so IAM Identity Center accounts in eu-central-1 hit q.eu-central-1
    // instead of the hardcoded us-east-1 host (which rejects region-bound tokens).
    const profileArn = credentials?.providerSpecificData?.profileArn;
    const m = typeof profileArn === "string"
      ? profileArn.toLowerCase().match(/^arn:aws:codewhisperer:([a-z0-9-]+):/)
      : null;
    const arnRegion = m?.[1];
    const storedRegion = (credentials?.providerSpecificData?.region || "us-east-1").trim().toLowerCase();
