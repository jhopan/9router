...
// The direct route returns the KIRO PAYLOAD, which has no top-level `systemPrompt`:
// the system text (thinking prefix + agentic protocol) is folded into the content
// of the current user turn because the CodeWhisperer surface 400s on a top-level
// systemPrompt. `systemPromptOf` therefore reads that content.
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("keeps the system instruction stable across turns while the timestamp stays fresh", () => {
    const first = C2K({
    ...
    // System text and the "[Context: Current time …]" note share one string (the
    // payload has no top-level systemPrompt), so compare the part BEFORE the
    // timestamp: the instruction must survive unchanged and must not carry the
    // current time, which lives in the trailing context note.
    const prefixOf = (out) => systemPromptOf(out).split("[Context: Current time")[0].trim();
