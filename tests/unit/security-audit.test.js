...
// This suite greps source files, so paths must resolve from the REPO ROOT, not
// from process.cwd(): vitest runs with cwd = tests/, which made every
// path.resolve("src/...") land on tests/src/... (ENOENT) whenever the suite was
// invoked from tests/ instead of the repo root.
...
describe("AUDIT-002: API key masking", () => {
  it("source should contain maskApiKey function", () => {
    const source = src("src/lib/db/repos/usageRepo.js");
...
describe("AUDIT-003: Proxy URL validation", () => {
  ...
  it("source should contain validateProxyUrl function", () => {
    const source = src("src/lib/network/outboundProxy.js");
...
describe("AUDIT-018: XSS escaping in OAuth callback", () => {
  it("source should contain escapeHtml function", () => {
    const source = src("src/lib/oauth/utils/server.js");
...
describe("AUDIT-004: Atomic lock file for MITM startup", () => {
  it("manager.js should define LOCK_FILE constant", () => {
    const source = src("src/mitm/manager.js");
